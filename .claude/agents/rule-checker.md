...
3. `${CLAUDE_PLUGIN_ROOT}/skills/novel-polish/references/12-axes.md` 축1~5의 탐지 키워드와 등급 기준

### Step 2: 에피소드 전문 정독 + grep 검증

grep으로 기계적 검출 먼저 → 정독으로 오탐 제거 + 추가 검출.
`${CLAUDE_PLUGIN_ROOT}/skills/novel-polish/references/12-axes.md` 축1~5의 탐지 키워드와 등급 기준을 참조한다.
