...
### Phase 0.5: design_dir 표준 검증 게이트 (필수, 자동 실행)

> Design Atlas 뷰어는 `projects/*/design/**/*.md` glob만 자동 수집한다.
> 비표준 위치(예: `docs/{name}/`)에 산출물을 두면 작가가 매번 마이그레이션해야 하므로,
> **큰 설계 시작 시점에 design_dir이 표준이 아니면 자동 보정**한다.

1. 프로젝트 루트의 `novel-config.md`를 확인한다(없으면 Step 4로).
2. `project.design_dir` 값을 추출한다.
3. **표준 형식 검증**: `^projects/[a-z0-9_-]+/design/$` 정규식과 일치하는가?
   - 일치 → 그대로 사용. Phase 0으로 진행.
   - 불일치(또는 누락) → Step 4로.
4. **자동 보정 (사용자에게 1줄 알림)**:
   ```
   ⚙️ design_dir이 비표준 위치를 가리키고 있어 표준 위치로 자동 보정합니다.
      이전: {기존 design_dir}
      변경: projects/{슬러그}/design/
      (Design Atlas 뷰어가 자동 인식하는 표준 경로)
   ```
   - novel-config.md의 `project.design_dir`과 모든 매핑 경로의 prefix를 일괄 치환한다
   - 기존 위치에 산출물이 있으면 `git mv`로 표준 위치로 옮긴다
   - 슬러그는 `project.episode_dir`의 `projects/{슬러그}/episode/` 패턴에서 추출하거나, 사용자에게 확인
5. 보정 완료 후 Phase 0으로 진행
...
| `_workspace/03_*_guide.md` | `{DESIGN_DIR}/{작품가제}_플롯훅가이드.md` |
   ...
   > Phase 5에서 config에 `{DESIGN_DIR}/{작품가제}_*.md`로 기록하므로, 여기서도 동일 경로에 저장한다.
   > `{DESIGN_DIR}` 디렉토리가 없으면 생성한다.
   > **표준값**: `projects/{슬러그}/design/` — Design Atlas 뷰어가 자동 수집하는 유일한 위치.
   > Phase 0.5에서 비표준 design_dir은 이미 표준으로 보정되었으므로 여기서는 그대로 사용한다.
...
project:
  ...
  episode_dir: "projects/{슬러그}/episode/"
  work_dir: "projects/{슬러그}/revision/"
  design_dir: "projects/{슬러그}/design/"   # ★ Design Atlas 자동 등록 표준 경로
