...
**측정 기준**: `LC_ALL=en_US.UTF-8 wc -m`으로 파일 전체 글자수를 측정한다 (마크다운 헤더·메타데이터 포함).

> **⚠️ locale 강제 필수**: macOS/Linux의 `wc -m`은 `LANG`/`LC_ALL`이 비어 있으면 character가 아닌 **byte**로 카운트한다. 한글 1자 = UTF-8 3 byte이므로 locale을 강제하지 않으면 한글 본문이 약 2~3배로 부풀려 보고된다 (예: 실제 3,000자 → 약 8,000으로 오인식). 반드시 `LC_ALL=en_US.UTF-8` 접두사를 붙여 실행한다.

1. **측정**: Bash `LC_ALL=en_US.UTF-8 wc -m {EPISODE_DIR}/ep{NNN}.md` → 글자수 확인
2. **판정**: 최종 목표 범위(기본 final_chars: 5000-8000) 대비 확인
   ...
   - **상한 초과** (8000자 초과 — 일반적 케이스):
     ...
     - 프롬프트에 다음을 포함:
       ```
       트리밍 모드:
       - 현재 글자수: {N}자 (`LC_ALL=en_US.UTF-8 wc -m` 기준, 헤더 포함)
...
3. Bash: `LC_ALL=en_US.UTF-8 wc -m {EPISODE_DIR}/ep{NNN}.md` → 목표 글자수 범위(기본 final_chars: 5000-8000) 확인. **`LC_ALL` 접두사 누락 금지** — 누락 시 한글이 byte로 카운트되어 약 2~3배로 부풀려 측정된다.
