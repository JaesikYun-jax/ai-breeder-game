...
**정식 제목**: 「D급 스킬 이세계 용사」  
**주인공**: 강지호 (28세, 삼류 프로그래머 → 이세계 용사)

---

## 프로젝트 개요

이세계 회귀 루프물 웹소설을 집필하고, 자체 제작 웹 리더앱으로 배포하는 1인 프로젝트.  
2026-04-13 게임(Phaser 3 비주얼노벨)에서 **선형 웹소설**로 피봇 완료.

**핵심 컨셉**: 코미디 이세계물의 외피 + 회귀 루프의 고통 — "코미디→고통" 낙차 공식  
**세계관**: 9대 지역, 10기둥 시스템, 순차 사망 구조 (아젤리아 → 솔라리스 → ... → 아비살)
...
│   ├── rule-checker.md            # 5축 규칙 위반 진단
│   ├── story-analyst.md           # 10축 서사 분석
│   ├── alive-enhancer.md          # ALIVE 4축 캐릭터 생동감
│   ├── revision-executor.md       # 15계층 우선순위 교정
│   └── revision-reviewer.md       # 교정 검증
└── skills/
...
```
Phase 1 (병렬):  rule-checker + story-analyst + alive-enhancer (19축 진단)
Phase 2 (순차):  revision-executor (15계층 우선순위 교정 + AITONE 자기검증)
Phase 3 (순차):  revision-reviewer (독립 검증 → PASS/REVISE)
```

**19축 품질 시스템:**
- Part A 위반 5축: BANNED, VOICE, TITLE, SILENCE, TRANS
- Part B 서사 10축: SCENE, LOGIC, UNIFORM, HOOK, OPENING, PACING, TONEDROP + 커스텀 3축 (REGRESSION, FORESHADOW, MODERN_REF)
- ALIVE 4축: 에코 대사, 침묵→비언어, 조연 고유 긴장, 관계 곡선
...
### 아크 구조 (novel-config.md 참조)
| 아크 | 범위 | 지역 | 상태 |
|------|------|------|------|
| arc1_azelia | ch001~ch005 | 아젤리아 왕국 | 초안 완료 |
| arc2_solaris | ch006~ch030 | 솔라리스 사막 | 연재 중 (13화) |
| arc3_kaizer ~ arc9_abyssal | 미정 | — | 미착수 |
...
4. 리더앱에서 읽기 → 피드백
5. /polish 파이프라인 실행 (5 에이전트, 19축 진단)
